...
Модель YOLOv8l-seg показывает хорошую точность на синтетических рентгеновских снимках, полученных из датасета VerSe. При инференсе на данных из той же выборки (train/val) ошибки возникают редко и, как правило, ограничиваются **смежными позвонками** — например, модель может перепутать L3 и L4 или T8 и T9. Это объясняется их **анатомическим сходством**: близкие размеры, форма и отсутствие ярко выраженных индивидуальных особенностей.
...
В совокупности эти факторы приводят к тому, что модель **не обобщает на данные, отличающиеся от тех, на которых она обучалась**. Это указывает на два возможных пути решения проблемы:  

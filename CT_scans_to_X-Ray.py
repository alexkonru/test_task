"""
Генерация синтетических рентгеновских снимков (DRR) из датасета VerSe
с последующей проекцией сегментации позвонков и созданием разметки в формате YOLO.
"""
...
# Пути и параметры обработки
WORK_DIR = "./verse" 
DATASET_VERSION = "20"
DATA_TYPE = "training"

# Параметры формирования DRR
DRR_CONFIG = {
    "hu_min": -500,          # Минимальное значение шкалы Хаунсфилда для отсечения
    "hu_max": 1500,          # Максимальное значение шкалы Хаунсфилда
    "attenuation_coeff": 0.03,  # Коэффициент ослабления для моделирования прохождения рентгеновских лучей
}

# Соответствие меток позвонков анатомическим названиям по спецификации VerSe
VERTEBRA_LABELS = {
...
def force_memory_cleanup():
    # Принудительная очистка памяти для стабильности
    gc.collect()
    gc.collect()
    time.sleep(1)

def get_orientation(affine):
    # Определение оси, соответствующей позвоночнику, на основе норм столбцов аффинной матрицы.
    # Предполагается, что наибольшая физическая протяжённость соответствует оси позвоночника.
    x_axis = affine[:3, 0]
    y_axis = affine[:3, 1]
    z_axis = affine[:3, 2]
...
def scale_to_physical(drr, seg_2d, voxel_sizes, projection="lateral"):
    # Масштабирование 2D-проекции для сохранения физических пропорций.
    # voxel_sizes = (dx, dy, dz) — размеры вокселя в миллиметрах.
    if projection == "axial":
        scale = voxel_sizes[0] / voxel_sizes[1]  # X / Y
    else:
        # Для боковой и передне-задней проекций: отношение продольной оси к поперечной
...
def download_verse_dataset():
    # Загрузка и распаковка датасета VerSe при отсутствии локальной копии
    print(f"\n{'='*50}")
...
def get_file_paths(patient_dir, base_dir):
    # Поиск файлов КТ и сегментации для одного пациента
...
def generate_drr(ct_volume, config, projection="lateral", spine_axis=2, x_axis_idx=0, y_axis_idx=1):
    # Генерация цифрового рентгеновского снимка (DRR) методом суммирования с ослаблением
    ...
    # Выбор оси проекции в зависимости от типа снимка
    if projection == "lateral":  # Боковая проекция — суммирование по оси X
        sum_axis = x_axis_idx
    elif projection == "ap":     # Передне-задняя — по оси Y
        sum_axis = y_axis_idx
    elif projection == "axial":  # Аксиальная — по оси позвоночника (Z)
        sum_axis = spine_axis
    else:
...
def project_segmentation(seg_volume, projection="lateral", spine_axis=2, x_axis_idx=0, y_axis_idx=1):
    # Проекция 3D-сегментации в 2D с выбором наиболее частой метки вдоль луча проекции
    seg_volume = seg_volume.astype(np.uint16)
    
    if projection == "lateral":
        axis1, axis2 = y_axis_idx, spine_axis  # Плоскость Y–Z
    elif projection == "ap":
        axis1, axis2 = x_axis_idx, spine_axis  # Плоскость X–Z
    elif projection == "axial":
        axis1, axis2 = x_axis_idx, y_axis_idx  # Плоскость X–Y
...
def create_yolo_labels(seg_2d, image_name, output_dir):
    # Создание разметки в формате YOLO с полигонами для каждого позвонка
    ...
    with open(label_path, "w") as f:
        for vert_id in vertebra_ids:
            ...
            if len(polygon) >= 3:
                class_id = vert_id - 1  # YOLO использует индексы с 0
                f.write(f"{class_id} " + " ".join(polygon) + "\n")
                count += 1
    
    return count

def process_single_patient_all_projections(patient_dir, base_dir, idx):
    # Обработка одного пациента: генерация трёх проекций (AP, lateral, axial)
    patient_name = patient_dir.name
    ...
    try:
        ...
        # Приведение сегментации к размеру КТ при несовпадении
